...
impl GeneratedCommit {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        let conventional_types = ["feat", "fix", "docs", "style", "refactor", "test", "chore", "perf", "ci", "build", "revert"];
        ...
        if let Some(colon_pos) = self.title.find(':') {
            ...
            if !conventional_types.contains(&clean_type) {
                return Err(format!("Invalid commit type '{}'. Valid types: {}", clean_type, conventional_types.join(", ")));
            }
